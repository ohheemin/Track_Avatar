...
# MOVING_AVG_N  = 10     # 이동평균 파라미터 (비활성화)
...
class JointFilter:
    def __init__(self, n_joints: int = 6,
        ...
        # self.buffers  = [deque(maxlen=window) for _ in range(n_joints)]  # 이동평균 비활성화
        self.last_pub  = [0.0] * n_joints
        # 3프레임 추세 판단을 위한 최근 raw값 이력 (조인트별, 최대 3개 보관)
    ...
    def update(self, thetas: list) -> list:
        ...
        for i, th in enumerate(thetas):
            # ── 3프레임 상승 추세 필터 ───────────────────────────────────────
            # trend_buf 에는 데드밴드 적용 전 raw값을 기록해 추세를 판단한다.
            self.trend_buf[i].append(th)

            candidate = th  # 기본값: 현재 raw값 그대로 사용
...
"""
    ...
    position은 칼만/이동평균 없이 데드밴드, 3프레임 추세 필터만 적용하고,
    effort는 관절 계산 끝난 후에 프레임 발행
...
def main():
    ...
    # print(f"Filter: Kalman(Q={KF_Q},R={KF_R})  MovingAvg(N={MOVING_AVG_N})  Deadband({DEADBAND_DEG:.1f}deg)")  # 칼만필터/이동평균 비활성화
    ...
    try:
        while True:
            ...
            y_ang += 4
            draw_text_with_bg(disp, "[pub] dead+trend", (w-230, y_ang),
            ...
            draw_text_with_bg(disp,
                f"Dead={DEADBAND_DEG:.1f}deg  Trend=3f",
